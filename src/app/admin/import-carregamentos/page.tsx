...
export default function ImportCarregamentosPage() {
  ...
  const [formData, setFormData] = useState<CarregamentoManual>({
    ...
    motorista: {
      ...
      dataInicio: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString().split('T')[0], // apenas data
    },
    horarios: {
      encostadoDoca: '',
      inicioCarregamento: '',
    ...
    timestamp: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString().slice(0, 16), // datetime-local formato
    dataCriacao: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString().slice(0, 16),
    dataEnvio: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString().slice(0, 16),
    dataAtualizacao: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString().slice(0, 16),
    mensagemDespacho: '',
    mensagemXPT: '',
  });
