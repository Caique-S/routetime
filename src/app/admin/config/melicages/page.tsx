...
const Toast = ({
  ...
  <div
    className={`mb-4 p-4 rounded flex justify-between items-center ${type === "success" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
  >
    <span>{message}</span>
    <button
      onClick={onClose}
...
export default function AdminConfigPage() {
  const [localConfig, setLocalConfig] = useState<LocalizacaoConfig | null>(
    null,
  );
  const [motoristaConfig, setMotoristaConfig] =
    useState<MotoristasConfig | null>(null);
  ...
  const saveXpt = async () => {
    ...
    if (isNaN(latitude) || isNaN(longitude) || isNaN(raio) || raio <= 0) {
      setXptError(
        "Latitude, longitude e raio devem ser números válidos (raio positivo).",
      );
      return;
    }
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-6xl mx-auto">
        <Link
          ...
          className="text-blue-400 self-center hover:text-blue-600 text-xl transition"
        >
          Voltar
        </Link>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLocalConfig((prev) =>
                    prev
                      ? { ...prev, coo_lat: parseFloat(e.target.value) || 0 }
                      : null,
                  )
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLocalConfig((prev) =>
                    prev
                      ? { ...prev, coo_lon: parseFloat(e.target.value) || 0 }
                      : null,
                  )
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLocalConfig((prev) =>
                    prev
                      ? { ...prev, raio: parseInt(e.target.value) || 0 }
                      : null,
                  )
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Refresh Lista (segundos)
              </label>
              <input
                type="number"
                value={motoristaConfig?.refresh_list || ""}
                onChange={(e) =>
                  setMotoristaConfig((prev) =>
                    prev
                      ? { ...prev, refresh_list: parseInt(e.target.value) || 0 }
                      : null,
                  )
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
                ...
                onChange={(e) =>
                  setMotoristaConfig((prev) =>
                    prev
                      ? {
                          ...prev,
                          refresh_route: parseInt(e.target.value) || 0,
                        }
                      : null,
                  )
                }
            ...
            <div className="flex items-center">
              <input
                ...
                onChange={(e) =>
                  setMotoristaConfig((prev) =>
                    prev ? { ...prev, monitoramento: e.target.checked } : null,
                  )
                }
          ...
          <div className="mt-4 flex justify-end">
            ...
            >
              {savingMotorista
                ? "Salvando..."
                : "Salvar Configuração Motoristas"}
            </button>
          </div>
        </div>
              ...
              {xpts.map((xpt) => (
                ...
                >
                  ...
                  <div className="mt-2 text-sm">
                    ...
                    {xpt.origin && (
                      <p className="text-gray-500">Origem: {xpt.origin}</p>
                    )}
                  </div>
