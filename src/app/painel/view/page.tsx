              ...
              <span className="text-xs font-bold text-blue-600 flex items-center gap-1">
                ...
                {motorista.docaNotifiedAt && (
                  <span className="text-gray-500 font-normal text-[13px]">
                    (às {new Date(motorista.docaNotifiedAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })})
                  </span>
                )}
              </span>
            )}
  ...
  {/* Tempo entre notificação e início */}
      {motorista.docaNotifiedAt && motorista.timestampInicioDescarga && (
        <div className="bg-yellow-50 rounded-lg px-3 py-2 mb-3 flex items-center justify-between">
          <span className="text-xs text-gray-500">⏱️ Tempo até início (notif → início)</span>
          <span className="font-mono font-bold text-yellow-700">
            {(() => {
              const diffSegundos = Math.max(0, Math.floor(
                (new Date(motorista.timestampInicioDescarga).getTime() - new Date(motorista.docaNotifiedAt).getTime()) / 1000
              ));
              const h = Math.floor(diffSegundos / 3600).toString().padStart(2, '0');
              const m = Math.floor((diffSegundos % 3600) / 60).toString().padStart(2, '0');
              const s = Math.floor(diffSegundos % 60).toString().padStart(2, '0');
              return `${h}:${m}:${s}`;
            })()}
          </span>
        </div>
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-screen-2xl mx-auto p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4">
          ...
          <Coluna titulo="Descarregando" motoristas={descarregando} cor="bg-blue-500" />
          <Coluna titulo="Finalizados" motoristas={finalizados} cor="bg-green-500" />
        </div>
      </div>
    </div>
  );
