...
import { LogIn, Truck, Scan, Settings } from 'lucide-react';
...
function persistirOperador(operador: OperadorData['operador']) {
  localStorage.setItem('operador_nome',  operador.nome);
  localStorage.setItem('operador_cargo', operador.cargo);
  localStorage.setItem('operador_data',  JSON.stringify(operador));
}
...
export default function Home() {
  ...
  // Redireciona para o dispatch se já estiver autenticado
  useEffect(() => {
    const nome  = localStorage.getItem('operador_nome');
    const cargo = localStorage.getItem('operador_cargo');
    if (nome && cargo) router.push('/dispatch');
    setIsAnimating(true);
  }, [router]);

  async function buscarOperador(id: string) {
    if (!id.trim()) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? 'Erro ao buscar operador');

      setOperadorData(data);
      if (data.operador) persistirOperador(data.operador);
      ...
      <div className={`max-w-md w-full space-y-8 z-10 transition-all duration-700 ${isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="text-center space-y-4">
          <div className="flex items-center justify-center space-x-3">
            ...
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Expedição</h1>
          </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 space-y-6 border border-gray-200/50">
          <div className="space-y-1">
            <label className="block text-sm font-medium text-gray-700">Código do Operador</label>
            <p className="text-xs text-gray-500">Digite o código único ou escaneie o QR Code</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="relative group">
              <input
                type="password"
                value={operadorId}
                onChange={(e) => { setOperadorId(e.target.value); setError(''); }}
                placeholder="Ex: 507f1f77bcf86cd799439011"
                className="w-full px-4 py-3 pr-12 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none text-gray-900 placeholder-gray-400 bg-white/50 group-hover:border-blue-400"
              />
              <button
                type="button"
                onClick={() => setShowScanner(true)}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-2 hover:bg-blue-50 rounded-lg transition-colors"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-linear-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:from-blue-400 disabled:to-blue-500 text-white font-semibold py-3 px-4 rounded-xl transition-all flex items-center justify-center gap-2 group shadow-lg hover:shadow-xl"
            >
              {loading ? (
                <>
          ...
          <div className="pt-6 border-t border-gray-100 flex items-center justify-center gap-4">
            ...
            <span className="text-xs text-gray-500">v1.05 • {new Date().getFullYear()}</span>
          </div>
        </div>
