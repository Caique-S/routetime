...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (motorista.status !== 'aguardando') {
      return NextResponse.json({ erro: 'Motorista não está aguardando', statusAtual: motorista.status }, { status: 400 });
    }

    const agora = new Date();
    const tempoFila = Math.floor((agora.getTime() - motorista.timestampChegada.getTime()) / 1000);

    await db.collection('melicages_motoristas').updateOne(
      { _id: new ObjectId(id) },
      { $set: { status: 'descarregando', timestampInicioDescarga: agora, tempoFila } }
    );

    const atualizado = await db.collection('melicages_motoristas').findOne({ _id: new ObjectId(id) });
    const { _id, ...rest } = atualizado!;
    return NextResponse.json({ success: true, message: 'Descarga iniciada', data: { ...rest, id: _id.toString() } });
  } catch (error: any) {
