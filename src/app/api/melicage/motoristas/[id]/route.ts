...
export async function PATCH(
  ...
  try {
    ...
    const allowed = ["doca", "status", "docaNotifiedAt", "gaiolas", "palets", "mangas"];
    const update: any = {};
    ...
    if (Object.keys(update).length === 0) {
      return NextResponse.json(
        { success: false, erro: "Nenhum campo válido para atualização" },
        { status: 400 },
      );
    }
    ...
    if (firestoreUpdate.updatedAt instanceof Date) {
      firestoreUpdate.updatedAt = firestoreUpdate.updatedAt.toISOString();
    }
